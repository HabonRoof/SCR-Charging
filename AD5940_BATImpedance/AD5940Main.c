...
{
  AppBATCfg_Type *pBATCfg;								/*Read the data at addr "pBATCfg", named AppBATCfg_Type pBATCfg is pointer AppBATCfg is struct*/
  AppBATGetCfg(&pBATCfg);									/*AppBATGetCfg is the address of pBATCfg, it's the pointer's pointer*/
  pBATCfg->SeqStartAddr = 0;							/*Get the SeqStartAddr from pBATCfg this struct*/
  pBATCfg->MaxSeqLen = 512;
  pBATCfg->RcalVal = 50;  							/* Value of RCAL on EVAL-AD5941BATZ board is 50mOhm */
  pBATCfg->ACVoltPP = 300.0f;							/* Pk-pk amplitude is 10mV ,Maximum is 300mV*/
  pBATCfg->DCVolt = 3200.0f;							/* Offset voltage of 3.5V*/
  ...
  pBATCfg->FifoThresh = 2;      					/* 2 results in FIFO, real and imaginary part. */
	...
	Datapoints = pBATCfg->SweepCfg.SweepPoints;
	ACVoltPP= pBATCfg->ACVoltPP;
	DCVolt = pBATCfg->DCVolt;
	FixFreq = pBATCfg->SinFreq;
	SweepEn = pBATCfg->SweepCfg.SweepEn;
}
